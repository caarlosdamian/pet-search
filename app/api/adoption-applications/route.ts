...
export async function POST(request: Request) {
  try {
    ...
    if (existingApplication) {
      return NextResponse.json({ error: "You have already applied to adopt this pet" }, { status: 409 })
    }
    ...
    await db
      .collection("users")
      .updateOne({ _id: new ObjectId(session.user.id) }, { $push: { applications: result.insertedId.toString() } })
